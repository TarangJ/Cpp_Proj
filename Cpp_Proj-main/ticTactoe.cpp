...
int main(){
	int board[9] = {0,0,0,0,0,0,0,0,0}; //starts with the empty board
	int possibleWinner;
	int move;
	bool isInvalid;
	string moveString;
	...
	while (true) {
		//Player X decides what will be the move
		do {
			cout << "X: ";
		...
		//check for game continue or not
		if (gameOver(board) > 0) {
			outputBoard(board);
			break;
		}

		//Player O will decide their move
		bool good = false;
		for (int x = 2; x > 0; x--){
		...
		//check for game will continue or not
